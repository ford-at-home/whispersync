...
try:
    ...
    workflow = None
    graph = None
    tool = lambda f: f  # Simple decorator passthrough
...
try:
    from .work_journal_agent import work_journal_tool, get_work_journal_agent
    from .memory_agent import memory_tool, get_memory_agent
    from .github_idea_agent import github_tool, get_github_idea_agent
except ImportError as e:
    logger.warning(f"Failed to import agent tools: {e}")
    # Use mock implementations if real agents not available
    try:
        from .agent_tools import work_journal_tool, memory_tool, github_tool
        get_work_journal_agent = lambda bucket=None: None
        get_memory_agent = lambda bucket=None: None
        get_github_idea_agent = lambda bucket=None: None
    except ImportError:
